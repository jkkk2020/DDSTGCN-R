...
def main():
    ...
    os.environ["CPU_DEVICE_ORDER"] = "PCI_BUS_ID"
    os.environ["CPU_VISIBLE_DEVICES"] = args.device
    ...
    G0 = torch.tensor(G0)
    G1 = torch.tensor(G1)
    H_T_new = torch.tensor(H_T_new)
    lwjl = lwjl
    indices = indices

    G0_all = torch.tensor(G0_all)
    G1_all = torch.tensor(G1_all)
    ...
    np.savez('predict_call_in_07_15_3.npz', prediction=prediction, ground_truth=ground_truth)
